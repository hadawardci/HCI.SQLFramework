...
using EasyDapper.Data;
using EasyDapper.Validation;
using EasyDapper.Values;
using PELEXMapper;
using System;
...
namespace EasyDapper.Model
{
    ...
    public abstract class DataContext<TConnection> : IDataContext
        where TConnection : DbConnection
    {
        ...
        public bool SaveOrUpdate<TEntity>(TEntity entity, bool isNavigated = true)
            where TEntity : class
        {
            var result = false;
            if (entity != null)
        ...
        private  bool IsUpdate<TEntity>(TEntity entity, string keyName) where TEntity : class
        {
            ...
            if (ImplementationUtil.ContainsInterface(entity.GetType(), nameof(IEntityState)))
                ...
                isExists = !new string[] { "0" ,""}.Contains(MapperUtil.GetValue(entity, keyName).ToString());
            else if (ImplementationUtil.ContainsProperty(entity.GetType(), "Id"))
                isExists = (entity as dynamic).Id > 0;
            else if (ImplementationUtil.ContainsProperty(entity.GetType(), entitiIdPattern))
                isExists = !new string[] { "0", "" }.Contains(MapperUtil.GetValue(entity, entitiIdPattern).ToString());
            return isExists;
        }
